...
def search_ids(data_json, stationCode, devTypeId):
    df = pd.DataFrame(columns=['ORIGEN','idSite', 'system_timestamp','formatted_system_timestamp','run_timestamp', 'formatted_run_timestamp', 'id', 'Device', 'description', 'formattedValue', 'rawValue'], dtype=object)
    data_dict = data_json['data'][0]['dataItemMap']
    system_timestamp = data_json['params']['currentTime']/1000
    formatted_system_timestamp = str(datetime.fromtimestamp(int(data_json['params']['currentTime']/1000)))
    run_timestamp = time.time()
    formatted_run_timestamp = str(datetime.fromtimestamp(run_timestamp).strftime('%Y-%m-%d %H:%M:%S'))
    ...
    for key in keys_of_interest:
        if key in data_dict and pd.notna(data_dict[key]):
            ...
            row = ["Fusión solar",stationCode, system_timestamp, formatted_system_timestamp, run_timestamp, formatted_run_timestamp, key, device_name, description, formatted_value, raw_value]
            df.loc[len(df)] = row
    return(df)
